...
.b {
	...
	<body><center><h1>你好！</h1><br>看起来你是第一次使用FredTools Homework CMS。<br>首次使用，我们需要一些信息以配置settings.php文件。<br>一般情况下，您的主机商会告诉您这些信息。<br>
  即使配置出错，您也可以按照example_settings.php中的教程配置。<br>现在，让我们开始吧。<br>首先，我们需要您的MySQL主机名。</center></body></html>";
  exit;
}
...
$sql = "SELECT userid,problemid,submitid,status,useranswer FROM status WHERE userid=".$_COOKIE["userid"];
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        //echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
        if($row['status'] == "RIGHT") $status=" class=success";
        else $status=' class=error';
    }
} else {
    echo "0 结果";
}
                ...
                while ($row = $result->fetch_assoc()) {
                    if(((stripos($jsteacher, $row['fromusername'])) != FALSE))
                      ...
                      echo '<tr'.$status.'>
								...
								<td>
									<button type="button" class="btn btn-info" onclick="document.href=\"http://homework.ascendto.top/probleminfo.php?id='.$row['problemid'].'\"">开始做题</button> 
								</td>
							</tr>';
                    else
                        if($jsteacher == '')
